...
ggplot(water) +
  ...
  geom_sf(data = nhoods, fill = NA, color = "black") +
...
## Plot 1: The number of libraries within 250m of public schools
...
ggplot(water) +
  geom_sf(fill = "lightblue", color = NA) +
  geom_sf(data = nhoods, fill = "grey92", color = "grey46") +
...
## Plot 2: The number and proportion of libraries within each neighborhood
9. The number and proportion of C polygons containing A points.
```{r}
nhoods <- nhoods %>%
...
ggplot(nhoods) +
  ...
  annotation_scale(location = "br") +
  theme_map() +
ggtitle("Neighborhoods by number of libraries") +
theme(panel.background = element_rect(fill = "gray98"),
        legend.position="right", legend.background = element_rect(fill = alpha("white", 0.5), color = "gray"))
...
## Plot 3: Neighborhoods with parks in them
13. The number and proportion of C polygons that overlap with D polygons.
```{r}
nhoods <- nhoods %>%
...
ggplot(water) +
  ...
  geom_sf(data = nhoods,
          aes(fill = has_parks)) +
 scale_fill_manual(values = c("orange", "darkseagreen1"),
...
## Plot 4: Neighborhoods with parks greater that 100,000 square meters
```{r}
...
## Plot 5: Neighborhoods by number of public schools
```{r}
nhoods <- nhoods %>%
...
ggplot(nhoods) +
  ...
  scale_fill_viridis_c(name = "Boston neighborhoods\nby number of public schools", breaks = breaks <- seq(0, 25, by = 5), labels = paste(prettyNum(breaks, big.mark = ","), "schools")) +
  annotation_scale(location = "br") +
  theme_map() +
ggtitle("Neighborhoods by number of schools") +
theme(panel.background = element_rect(fill = "gray98"),
        legend.position="right", legend.background = element_rect(fill = alpha("white", 0.5), color = "gray"))
```


## Plot 5:Average density of schools in each neighborhood
```{r}
nhoods <- nhoods %>%
  mutate(num_schools = lengths(st_covers(nhoods, schools)))
...
ggplot(nhoods) +
  ...
  annotation_scale(location = "br") +
  theme_map() +
ggtitle("Neighborhoods by density of public schools") +
theme(panel.background = element_rect(fill = "gray98"),
        legend.position="right", legend.background = element_rect(fill = alpha("white", 0.5), color = "gray"))
...
## Plot 7: Density of libraries in each neighborhoods
```{r}
nhoods <- nhoods %>%
  mutate(num_libraries = lengths(st_covers(nhoods, libraries)))
...
ggplot(nhoods) +
  ...
  scale_color_viridis_c(direction = -1, name = 
                          "Boston public schools \nby distance to a library", breaks = breaks <- seq(0, 2500, by = 500),) +
  theme_map() +
