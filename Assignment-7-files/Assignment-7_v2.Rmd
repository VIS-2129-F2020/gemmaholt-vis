...
nhood_map <- leaflet(somerville_nhoods) %>%
...
saveWidget(nhood_map, file = "nhood_map.html")
```
...
density_map <- leaflet(nhood_rats) %>%
  addProviderTiles(providers$Stamen.TonerLite) %>%
...
saveWidget(density_map, file = "density_map.html")
```
...
saveWidget(rat_map, file = "rat_map.html")
```
...
ratster_map <- leaflet(nhood_points) %>%
  addProviderTiles(providers$Stamen.TonerLite) %>%
...
saveWidget(ratster_map, file = "ratster_map.html")
```
